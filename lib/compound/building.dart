...
// Placeholder data (replace with actual data)
 const List<Map<String, dynamic>> zoneRanges = [
  {'start': 1, 'end': 70, 'zone': 'زوون (1) الأكاسيا'},
  {'start': 71, 'end': 179, 'zone': 'زوون (2) القرنفل'},
  {'start': 180, 'end': 226, 'zone': 'زوون (3) البنفسج'},
  {'start': 227, 'end': 286, 'zone': 'زوون (4) الأوركيد'},
  {'start': 287, 'end': 345, 'zone': 'زوون (5) الجوليت'},
  {'start': 346, 'end': 400, 'zone': 'زوون (6) التوليب'},
  {'start': 401, 'end': 452, 'zone': 'زوون (7) الريحان'},
  {'start': 453, 'end': 522, 'zone': 'زوون (8) اللوتس'},
  {'start': 523, 'end': 584, 'zone': 'زوون (9) الياسمين'},
  {'start': 585, 'end': 636, 'zone': 'زوون (10) البانسيه'},
  {'start': 637, 'end': 689, 'zone': 'زوون (11) اللافندر'},
];

 const Map<int, String> predefinedLinks = {
  ...
  679: 'https://goo.gl/maps/ze6b7xnDbUbV1rJ6',
  680: 'https://goo.gl/maps/5qY8V7zKxL3mP9nQ7',
  681: 'https://goo.gl/maps/2tR6X8vN4kW5J9mV7',
  682: 'https://goo.gl/maps/7yF3K9qP8mT2N5vU6',
  683: 'https://goo.gl/maps/3xP7V8tR9nY6K4wQ7',
  684: 'https://goo.gl/maps/6vQ8W9xS2mZ7L3tR6',
  685: 'https://goo.gl/maps/8tR9Y2vU3nX8M4wQ7',
  686: 'https://goo.gl/maps/2xQ9Z3wV4mY7N5tR6',
  687: 'https://goo.gl/maps/5yT2X8vW6nZ9P4wQ7',
  688: 'https://goo.gl/maps/7xR3Y9tU8mW2Q5vT6',
  689: 'https://goo.gl/maps/9tQ4Z2wV5nX3R6wQ7',
};
...
class _BuildingsScreenState extends State<BuildingsScreen> {
  ...
  Map<String, dynamic>? _findZone(int buildingNumber) {
    for (var range in zoneRanges) {
      if (buildingNumber >= range['start'] && buildingNumber <= range['end']) {
        return {
          'zone': range['zone'],
          'link': predefinedLinks[buildingNumber],
        };
      }
    }
    return null;
  ...
  void _showBuildingsBottomSheet(BuildContext context, Map<String, dynamic> zone) {
    final buildings = List.generate(
      zone['end'] - zone['start'] + 1,
          (index) => {
        'name': 'العماره رقم: ${zone['start'] + index}',
        'zone': zone['zone'],
        'location': predefinedLinks[zone['start'] + index],
      },
    );
              ...
              SliverPadding(
                ...
                sliver: SliverToBoxAdapter(
                  child: _searchQuery.isEmpty
                    ...
                    itemCount: zoneRanges.length,
                    itemBuilder: (context, index) {
                      final zone = zoneRanges[index];
                      return FadeInUp(
                        duration: Duration(milliseconds: 400 + (index * 150)),
                        child: ZoneCard(
                          zone: {
                            'name': zone['zone'],
                            'range': 'العمائر من ${zone['start']} إلى ${zone['end']}',
                            'start': zone['start'],
                            'end': zone['end'],
                          },
                          cardColor: theme.cardColors[index % theme.cardColors.length],
                          onTap: () => _showBuildingsBottomSheet(context, zone),
                        ),
                      );
                    },
                  )
  ...
  Widget _buildSearchResult() {
    final theme = ThemeManager().currentTheme;
    final int? buildingNumber = int.tryParse(_searchQuery);
    if (buildingNumber == null) {
      return Center(
        child: Text(
          'يرجى إدخال رقم مبنى صحيح',
          style: GoogleFonts.cairo(
            fontSize: 16,
            color: theme.secondaryTextColor,
            fontWeight: FontWeight.w500,
    ...
    final zoneInfo = _findZone(buildingNumber);
    if (zoneInfo == null) {
      return Center(
        child: Text(
          'لم يتم العثور على العماره',
          style: GoogleFonts.cairo(
            fontSize: 16,
            color: theme.secondaryTextColor,
            fontWeight: FontWeight.w500,
    ...
    return GridView.builder(
      ...
      physics: const NeverScrollableScrollPhysics(),
      gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(
        maxCrossAxisExtent: MediaQuery.of(context).size.width < 600 ? 400 : 380,
        mainAxisSpacing: 20,
        crossAxisSpacing: 20,
        childAspectRatio: MediaQuery.of(context).size.width < 600 ? 1.4 : 0.85,
      ),
      itemCount: 1,
      itemBuilder: (context, index) {
        return FadeInUp(
          ...
          child: BuildingCard(
            building: {
              'name': 'العماره رقم: $buildingNumber',
              'zone': zoneInfo['zone'],
              'location': zoneInfo['link'],
            },
            cardColor: theme.cardColors[0],
          ),
        );
      },
    );
...
class _ZoneCardState extends State<ZoneCard> with SingleTickerProviderStateMixin {
                        ...
                        children: [
                          Expanded(
                            child: ElasticIn(
                              duration: ThemeManager.animationDuration,
                              child: Text(
                                widget.zone['name']?.toString() ?? 'غير معروف',
                                style: GoogleFonts.cairo(
                      ...
                      FadeInUp(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                widget.zone['range']?.toString() ?? 'غير محدد',
                                style: GoogleFonts.cairo(
...
class _BuildingCardState extends State<BuildingCard> with SingleTickerProviderStateMixin {
  ...
  void _shareBuildingDetails() {
   final name = widget.building['name']?.toString() ?? 'غير معروف';
   final zone = widget.building['zone']?.toString() ?? 'غير محدد';
   final location = widget.building['location']?.toString() ?? 'غير متوفر';

   final shareText = '''
🌟 تفاصيل العماره من جاردينيا توداي 🌟
🏢 $name
📍 $zone
🗺️ $location
...
''';
   ...
   Share.share(shareText.trim(), subject: 'تفاصيل العماره: $name');
  }
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return DetailsBottomSheet(
          ...
          onMap: () => _launchMap(context, widget.building['location']?.toString()),
          onShare: _shareBuildingDetails,
        );
      },
    );
  }
                        ...
                        children: [
                          Expanded(
                            child: ElasticIn(
                              duration: ThemeManager.animationDuration,
                              child: Text(
                                widget.building['name']?.toString() ?? 'غير معروف',
                                style: GoogleFonts.cairo(
                                  fontSize: isMobile ? 22 : 24,
                                  fontWeight: FontWeight.w800,
                                  color: theme.textColor,
                      ...
                      FadeInUp(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                widget.building['zone']?.toString() ?? 'غير محدد',
                                style: GoogleFonts.cairo(
                                  fontSize: isMobile ? 15 : 16,
                                  color: theme.secondaryTextColor,
                                  fontWeight: FontWeight.w600,
...
class BuildingsBottomSheet extends StatefulWidget {
  final List<Map<String, dynamic>> buildings;

  const BuildingsBottomSheet({
    super.key,
    required this.buildings,
  });

  @override
  _BuildingsBottomSheetState createState() => _BuildingsBottomSheetState();
}

class _BuildingsBottomSheetState extends State<BuildingsBottomSheet> with TickerProviderStateMixin {
  late AnimationController _sheetController;
  late Animation<Offset> _sheetAnimation;
                        ...
                        Expanded(
                          child: ElasticIn(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                'العمائر في ${widget.buildings.isNotEmpty ? widget.buildings[0]['zone'] : 'غير معروف'}',
                                style: GoogleFonts.cairo(
                                  fontSize: isMobile ? 22 : 24,
                                  fontWeight: FontWeight.w700,
                                  color: theme.textColor,
