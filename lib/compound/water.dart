...
class _WaterMeterScreenState extends State<WaterMeterScreen> {
  ...
  final List<Map<String, dynamic>> waterDetails = [
    {
      'title': 'شركة مدكور',
    ...
    {
      'title': 'طلب عداد',
      'subtitle': 'إجراءات الحصول على عداد مياه',
  ...
  Widget build(BuildContext context) {
    ...
    final filteredDetails = waterDetails.where((detail) {
...
class _ContactCardState extends State<ContactCard> with SingleTickerProviderStateMixin {
  ...
  void _shareDetail() {
    ...
    String shareText = '''
🌟 تفاصيل عداد المياه من جاردينيا توداي 🌟
📋 $title
ℹ️ $subtitle
''';
...
class _DetailsBottomSheetState extends State<DetailsBottomSheet> with TickerProviderStateMixin {
  ...
  Widget _buildActionButton({
    ...
    Color backgroundColor = Colors.blue, // Default color, will be overridden by theme
  }) {
    final theme = ThemeManager().currentTheme;
    final isMobile = MediaQuery.of(context).size.width < 600;
          ...
          child: Column(
            ...
            children: [
              FadeInUp(
                duration: ThemeManager.animationDuration,
                child: Text(
                  'إجراءات الحصول على عداد المياه:',
                  style: GoogleFonts.cairo(
